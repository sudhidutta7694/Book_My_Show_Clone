...
<template>
    ...
    <div class="bg-slate-800 content p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-8">
      <div class="flex flex-col justify-center items-center bg-slate-700 rounded-xl hover:scale-105" v-for="movie in movies" :key="movie.id">
        <router-link to="/overview/:id"><img :src="getImageUrl(movie.poster_path)" :alt="movie.title" class="rounded-lg shadow-lg"></router-link>
