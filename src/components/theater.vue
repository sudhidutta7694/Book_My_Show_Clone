  ...
  <div class="bg-slate-700 text-gray-300 min-h-screen">
    <div class="theater-list pl-12 py-40">
      <div class="flex justify-center items-center">
        <h1 class="text-4xl font-bold inline mb-16">Theaters Streaming <span class="font-extrabold mb-16">'{{ movie }}'</span></h1>
      </div>
      <div class="flex justify-between item-center">
        <div class="mt-20 ml-48 justify-center items-center">
          <div class="filter mt-16 mb-12 flex">
            <select id="day-filter" v-model="selectedDay" class="p-4 text-xl font-serif font-semibold border border-spacing-x-3 border-red-300 bg-slate-900 rounded flex gap-4">
              <option value="" hidden>Please Select a Date</option>
              <option class="m-4 text-xl font-serif font-semibold border border-red-600 bg-slate-900 rounded-full" v-for="day in filteredDays" :key="day.value.slice(0, -7)" :value="day.value.slice(0, -7)">{{ day.value }}</option>
            </select>
          </div>
          <ul v-if="selectedDay" class="fade-enter-active">
            <li v-for="(theater, index) in filteredTheaters" :key="theater.id" :style="getTheaterAnimation(index)" class="mb-12 fade-enter">
              <div class="theater-item flex justify-between bg-slate-800 p-6 rounded-xl w-[30vw] transition-all ease-linear">
                <div class="">
                  <h3 class="text-xl font-semibold">{{ theater.name }}</h3>
                  <p class="text-green-200">Timing: {{ theater.timing }}</p>
                </div>
                <routerLink :to="{ name: 'hall', query: { theater: JSON.stringify(theater), movie: movie } }" @click="addToUserTheaters(theater)" class="bg-red-700 hover:bg-red-800 text-gray-100 font-bold py-2 px-4 rounded">
                  Book Seats
                </routerLink>
              </div>
            </li>
        ...
        <img class="mr-48" src="https://media1.giphy.com/media/g6UKQJ1y9Bh0RH2pOY/giphy.gif?cid=6c09b952c22b1418a908ebcad9c45e35003baa8909b7f776&ep=v1_internal_gifs_gifId&rid=giphy.gif&ct=s" alt="ticket_gif" />
      </div>
    </div>
  </div>
</template>
